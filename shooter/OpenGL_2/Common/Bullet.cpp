...
BULLET::BULLET()
{
	m_Points[0] = vec4(-0.03f, 0.03f, 0.0f, 1.0f);
	m_Points[1] = vec4(0.03f, 0.03f, 0.0f, 1.0f);
	m_Points[2] = vec4(0.03f, -0.03f, 0.0f, 1.0f);
	m_Points[3] = vec4(-0.03f, 0.03f, 0.0f, 1.0f);
	m_Points[4] = vec4(0.03f, -0.03f, 0.0f, 1.0f);
	m_Points[5] = vec4(-0.03f, -0.03f, 0.0f, 1.0f);
...
void BULLET::SetShader(mat4 &mxView, mat4 &mxProjection, GLuint uiShaderHandle)
{
	...
	if (uiShaderHandle == MAX_UNSIGNED_INT) m_uiProgram = InitShader("vsVtxColor_Bullet.glsl", "fsVtxColor.glsl");
	...
	m_uiScale = glGetUniformLocation(m_uiProgram, "fScale");
}
...
void BULLET::SetTransAndScale(float fTransX, float fTransY)
{
	glUseProgram(m_uiProgram);
...
void BULLET::SetTransAndScale(float fTransX, float fTransY, float fScale)
{
	glUseProgram(m_uiProgram);
	glUniform1f(m_uiTransX, fTransX);
	glUniform1f(m_uiTransY, fTransY);
	glUniform1f(m_uiScale, fScale);
}

void BULLET::MyBulletMotion(float dt)
{
	SetTransAndScale(BulletX, BulletY += dt);
}

void BULLET::EnemyABulletMotion(float dt)
{
	SetTransAndScale(BulletX, BulletY -= dt);
}
#include "Enemy_TypeA.h"

ENEMY_A::ENEMY_A()
{
	m_Points[0] = vec4(0.0f, 0.05f, 0.0f, 1.0f);
	m_Points[1] = vec4(0.0f, -0.15f, 0.0f, 1.0f);
	m_Points[2] = vec4(0.15f, 0.15f, 0.0f, 1.0f);
	m_Points[3] = vec4(0.0f, 0.05f, 0.0f, 1.0f);
...
void ENEMY_A::CreateBufferObject()
{
...
void ENEMY_A::SetShader(mat4 &mxView, mat4 &mxProjection, GLuint uiShaderHandle)
{
	// Load shaders and use the resulting shader program
	if (uiShaderHandle == MAX_UNSIGNED_INT) m_uiProgram = InitShader("vsVtxColor_Plane.glsl", "fsVtxColor.glsl");
	...
	m_uiRotate = glGetUniformLocation(m_uiProgram, "fAngle");
}

void ENEMY_A::SetViewMatrix(mat4 &mat)
{
	m_mxView = mat;
	m_bUpdateMV = true;
}

void ENEMY_A::SetProjectionMatrix(mat4 &mat)
{
	m_mxProjection = mat;
	m_bUpdateProj = true;
}

void ENEMY_A::SetTRSMatrix(mat4 &mat)
{
	m_mxTRS = mat;
	m_bUpdateMV = true;
}

void ENEMY_A::SetColor(GLfloat vColor[4])
{
	for (int i = 0; i < 6; i++) {
...
void ENEMY_A::SetVtxColors(GLfloat vLFColor[], GLfloat vLRColor[], GLfloat vTRColor[], GLfloat vTLColor[])
{
...
void ENEMY_A::SetTransAndRotate(float fTransX, float fTransY)
{
	glUseProgram(m_uiProgram);
...
void ENEMY_A::SetTransAndRotate(float fTransX, float fTransY, float fAngle)
{
	glUseProgram(m_uiProgram);
	glUniform1f(m_uiTransX, fTransX);
	glUniform1f(m_uiTransY, fTransY);
	glUniform1f(m_uiRotate, fAngle);
}

void ENEMY_A::DetectBullet(BULLET MyBullet)
{
	if (((MyBullet.BulletX >= (EnemyAPosX - HitRange)) && (MyBullet.BulletX <= (EnemyAPosX + HitRange))) && ((MyBullet.BulletY >= (EnemyAPosY - HitRange)) && (MyBullet.BulletY <= (EnemyAPosY + HitRange))))
	{
		CrashFlag = true;
	}
}

#include "BackgroundStar_Small.h"
...
SMALLSTAR::SMALLSTAR()
{
	for (int i = 0; i < QUAD_NUM; i++) {
		m_Points[i] = vec4((RADIUS * cosf(M_PI*2.0f*i / QUAD_NUM)), (RADIUS * sinf(M_PI*2.0f*i / QUAD_NUM)), 0.0f, 1.0f);
...
void SMALLSTAR::CreateBufferObject()
{
...
void SMALLSTAR::SetShader(mat4 &mxView, mat4 &mxProjection, GLuint uiShaderHandle)
{
	// Load shaders and use the resulting shader program
	if (uiShaderHandle == MAX_UNSIGNED_INT) m_uiProgram = InitShader("vsVtxColor_Star.glsl", "fsVtxColor.glsl");
...
void SMALLSTAR::SetViewMatrix(mat4 &mat)
{
	m_mxView = mat;
	m_bUpdateMV = true;
}

void SMALLSTAR::SetProjectionMatrix(mat4 &mat)
{
	m_mxProjection = mat;
	m_bUpdateProj = true;
}

void SMALLSTAR::SetTRSMatrix(mat4 &mat)
{
	m_mxTRS = mat;
	m_bUpdateMV = true;
}

void SMALLSTAR::SetColor(GLfloat vColor[4])
{
	for (int i = 0; i < 6; i++) {
...
void SMALLSTAR::SetVtxColors(GLfloat vLFColor[], GLfloat vLRColor[], GLfloat vTRColor[], GLfloat vTLColor[])
{
...
void SMALLSTAR::SetTrans(float fTransX, float fTransY)
{
	glUseProgram(m_uiProgram);

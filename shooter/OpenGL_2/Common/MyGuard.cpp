...
MYGUARD::MYGUARD()
{
	m_Points[0] = vec4(-0.1f, 0.1f, 0.0f, 1.0f);
	m_Points[1] = vec4(-0.1f, -0.1f, 0.0f, 1.0f);
	m_Points[2] = vec4(0.1f, 0.1f, 0.0f, 1.0f);
	m_Points[3] = vec4(0.1f, 0.1f, 0.0f, 1.0f);
...
void MYGUARD::CreateBufferObject()
{
...
void MYGUARD::SetShader(mat4 &mxView, mat4 &mxProjection, GLuint uiShaderHandle)
{
	// Load shaders and use the resulting shader program
	if (uiShaderHandle == MAX_UNSIGNED_INT) m_uiProgram = InitShader("vsVtxColor_guard.glsl", "fsVtxColor.glsl");
...
void MYGUARD::SetViewMatrix(mat4 &mat)
{
	m_mxView = mat;
	m_bUpdateMV = true;
}

void MYGUARD::SetProjectionMatrix(mat4 &mat)
{
	m_mxProjection = mat;
	m_bUpdateProj = true;
}

void MYGUARD::SetTRSMatrix(mat4 &mat)
{
	m_mxTRS = mat;
	m_bUpdateMV = true;
}

void MYGUARD::SetColor(GLfloat vColor[4])
{
	for (int i = 0; i < 6; i++) {
...
void MYGUARD::SetVtxColors(GLfloat vLFColor[], GLfloat vLRColor[], GLfloat vTRColor[], GLfloat vTLColor[])
{
...
void MYGUARD::SetTrans(float fTransX, float fTransY)
{
	glUseProgram(m_uiProgram);
...
void MYGUARD::SetTransAndRotate(float fTransX, float fTransY, float fAngle)
{
	glUseProgram(m_uiProgram);
	glUniform1f(m_uiTransX, fTransX);
	glUniform1f(m_uiTransY, fTransY);
